...
import de.tudortmund.cs.iltis.folalib.grammar.construction.builder.LeftRegularGrammarBuilder;
 ...
 * LeftRegularGrammarBuilder} to view the exact definition of this type of grammar. This class takes
 * a grammar of any kind (even with obvious faults as e.g. using symbols that are not defined in the
 * alphabets) and tries to convert it into a proper type 3 grammar. If that is not possible, a
 ...
 */
public class GrammarToLeftRegularGrammarSpecialization<
                T extends Serializable, N extends Serializable>
        implements IGrammarSpecialization<
                T, N, LeftRegularProduction<T, N>, Grammar<T, N, LeftRegularProduction<T, N>>> {

    @Override
    public Result<Grammar<T, N, LeftRegularProduction<T, N>>, GrammarConstructionFaultCollection>
     ...
     * right-regular RHS, a respective fault gets added.
     */
    private List<Fault<GrammarConstructionFaultReason>> getRhsFaults(
        ...
        for (Production<T, N> production : grammarToSpecialize.getProductions()) {
            ...
            if (production.getRhs().size() == 2
                ...
                // Any arbitrary non-left-regular production
                faults.add(
                        new ProductionFault<>(
    ...
    /** Converts the grammar to a real left-regular grammar. */
    @SuppressWarnings("OptionalGetWithoutIsPresent")
    private Grammar<T, N, LeftRegularProduction<T, N>> convertGrammar(
            Grammar<T, N, ? extends Production<T, N>> grammarToSpecialize) {
        // Cast to regular production grammar if no faults were found
        List<LeftRegularProduction<T, N>> regularProductions =
                grammarToSpecialize.getProductions().stream()
                        .map(LeftRegularProduction::leftRegularProductionFromProduction)
